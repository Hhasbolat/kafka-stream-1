...
public class KafkaBankResponseProcessor {
    ...
    public KStream<String, BankTransaction> kStreamItem(StreamsBuilder builder) {
        ...
        var bankResponseJsonSerde = new JsonSerde<>(BankResponse.class);
        var bankTransactionJsonSerde = new JsonSerde<>(BankTransaction.class);

        KStream<Windowed<String>, BankTransaction> stream = builder
            .stream("bank-response", Consumed.with(stringSerde, bankResponseJsonSerde))
            .map((key, value) -> KeyValue.pair(value.getBankCode().toString(), value))
            .groupByKey(Grouped.with(stringSerde, bankResponseJsonSerde))
        ...
        map.to("bank-transaction", Produced.with(Serdes.String(), bankTransactionJsonSerde));
